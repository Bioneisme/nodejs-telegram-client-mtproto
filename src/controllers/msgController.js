"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const msgMethods_1 = __importDefault(require("../methods/msgMethods"));
function delay(ms) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield new Promise(resolve => setTimeout(resolve, ms));
    });
}
class MsgController {
    sendMessageToUser(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { user_id, message, count, interval } = req.body;
                for (let i = 0; i < count; i++) {
                    yield delay(interval);
                    const result = yield msgMethods_1.default.sendMessageToUser(user_id, message);
                    if (result.error) {
                        return res.render('pages/index', { error: result.message });
                    }
                }
            }
            catch (e) {
                next(e);
            }
        });
    }
    ;
    sendMessageToChat(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { chat_id, message, count, interval } = req.body;
                for (let i = 0; i < count; i++) {
                    yield delay(interval);
                    const result = yield msgMethods_1.default.sendMessageToChat(chat_id, message);
                    if (result.error) {
                        return res.render('pages/index', { error: result.message });
                    }
                }
            }
            catch (e) {
                next(e);
            }
        });
    }
    ;
}
exports.default = new MsgController();
